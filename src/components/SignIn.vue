...
<template>
  <div class="flex min-h-full flex-1 flex-col sm:px-6 lg:px-8">
    ...
    <div class="sm:mx-auto sm:w-full mt-10 sm:max-w-md">
      <h2 class="mt-6 text-center text-2xl font-bold leading-9 tracking-tight text-gray-900">{{ $t('frontPage.header') }}
      </h2>
    </div>
        ...
        <div v-if="loggingMethods.email === true">
          <router-link to="/formview" custom v-slot="{ navigate }">
            <button type="submit" @click="navigate"
              class="loginBtn loginBtn--email flex border-0 w-full justify-center rounded-md bg-indigo-600 px-3 py-1.5 text-sm font-semibold leading-6 text-white shadow-sm hover:bg-indigo-500 ">{{
                $t('frontPage.emailButton') }}</button>
          </router-link>
        </div>
